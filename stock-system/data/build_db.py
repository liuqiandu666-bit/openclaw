...
def prune_stale_fetch_log(stocks):
    """Drop fetch_log rows for stocks no longer in the current active universe."""
    ...
    count = conn.execute(
        f"SELECT COUNT(*) FROM fetch_log WHERE code NOT IN ({placeholders})",
        codes,
    ).fetchone()[0]
    if count:
        conn.execute(f"DELETE FROM fetch_log WHERE code NOT IN ({placeholders})", codes)
        conn.commit()
    conn.close()
    return count
...
def run(retry_only=False, n_workers=DEFAULT_WORKERS):
    ...
    pruned = prune_stale_fetch_log(stocks)
    if pruned:
        log(f"已清理 {pruned} 条退市/失效股票的旧失败记录")
